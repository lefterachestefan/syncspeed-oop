...
SyncAction::SyncAction(Sync::Action action) : action(action) {}

const Sync::Action& SyncAction::get_action() const {
    return action;
}
...
std::string SyncAction::get_type_string() const {
    return std::visit([](auto&& arg) -> std::string {
        ...
        if constexpr (std::is_same_v<T, Sync::CreateDir>) return "CreateDir";
        else if constexpr (std::is_same_v<T, Sync::DeleteDir>) return "DeleteDir";
        else if constexpr (std::is_same_v<T, Sync::UpdateFile>) return "UpdateFile";
        else if constexpr (std::is_same_v<T, Sync::DeleteFile>) return "DeleteFile";
        else if constexpr (std::is_same_v<T, Sync::ConflictFile>) return "ConflictFile";
        return "Unknown";
    }, action);
}

std::filesystem::path SyncAction::get_path() const {
    return std::visit([](auto&& arg) -> std::filesystem::path {
        return arg.relative_path;
    }, action);
}
...
void compute_diff_impl(const Directory &local, const Directory &remote,
  ...
  for (const auto &[name, l_file] : local_files) {
    ...
    if (r_it != remote_files.end() &&
        r_it->second->get_hash() != l_file->get_hash()) {
...
std::vector<SyncAction> compute_diff(const Directory &local,
