...
void DirectoryWatcher::add_watches_recursive(
...
void DirectoryWatcher::start(std::function<void()> on_change) {
  ...
  running = true;
  watch_thread =
      std::thread([this, on_change]() { this->watch_loop(on_change); });
}
...
bool DirectoryWatcher::is_running() const {
    return running;
}

const std::filesystem::path& DirectoryWatcher::get_watched_path() const {
    return root_path;
}

std::ostream& operator<<(std::ostream& os, const DirectoryWatcher& watcher) {
    os << "DirectoryWatcher(path=" << watcher.root_path << ", running=" << (watcher.running ? "yes" : "no") << ")";
    return os;
}
