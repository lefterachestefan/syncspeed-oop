...
SyncSession::SyncSession(const std::filesystem::path &local_path) : local_sync_folder(local_path) {}

const std::filesystem::path& SyncSession::get_local_path() const {
    return local_sync_folder;
}

std::expected<void, std::string>
send_actions(NetworkConnection &conn,
  ...
  for (const auto &sync_action : actions) {
    ...
    if (std::holds_alternative<Sync::DeleteFile>(action)) {
      ...
      std::filesystem::create_directories(local_sync_folder /
                                          act.relative_path);
      ...
      if (is_conflict) {
        std::filesystem::rename(target_path,
                                target_path.string() + ".conflict");
      }
...
std::expected<void, std::string>
serve_requests(NetworkConnection &conn,
               const std::filesystem::path &local_sync_folder) {
  while (true) {
...
std::expected<void, std::string>
SyncSession::run_client_side(NetworkConnection &conn) {
...
std::expected<void, std::string>
SyncSession::run_server_side(NetworkConnection &conn) {
...
std::expected<void, std::string>
SyncSession::server_sync(NetworkConnection &conn, const std::filesystem::path &local_sync_folder) {
    SyncSession session(local_sync_folder);
    return session.run_server_side(conn);
}

std::expected<void, std::string>
SyncSession::client_sync(NetworkConnection &conn, const std::filesystem::path &local_sync_folder) {
    SyncSession session(local_sync_folder);
