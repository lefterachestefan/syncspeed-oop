...
SyncedFolder::SyncedFolder(const std::filesystem::path &path) : path(path) {}

SyncedFolder::~SyncedFolder() {
    // std::cout << "SyncedFolder destructor for " << path << "\n";
}

std::expected<SyncedFolder, SyncDirectoryError>
SyncedFolder::try_create(const std::filesystem::path &path) {
...
std::expected<void, FileError> SyncedFolder::rescan() {
    return Directory::try_create(path).and_then(
        [this](auto dir) -> std::expected<void, FileError> {
...
const Directory* SyncedFolder::get_top_folder() const {
    return top_folder.get();
}
